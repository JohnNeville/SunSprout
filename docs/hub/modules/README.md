...
| `VBAT_RAW` | The battery connector `J4`, before the reverse-polarity FET `Q1`. |
| `VBAT_PROTECTED` | The pack side of the battery path. It sits after `Q1` and before the ship FET `Q3`. The fuel gauge connects here. |
| `VBAT` | The system side of the battery path. It sits after `Q3` and feeds the charger `BAT` pins. |
...
- The ship FET `Q3` uses the same AO3400A as the four input-multiplexer FETs, so adding it cost
  a placement but no new part number.
...
| [Battery charger](bq25798-charger.md) | `U8`, `Q3`, `Q301`–`Q304`, `L1` | BQ25798RQMR |
...
| [Input reverse-polarity protection](input-protection.md) | `Q1`, `Q2`, `D2` | MDD3415, DMP3098L-7, BZT52C12S |
